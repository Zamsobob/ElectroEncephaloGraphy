...
for s = 1:numsubjects % CHANGE TO FOR IF PARALLEL COMPUTING TOOLBOX IS NOT INSTALLED
   ...
   % IMPORT EVENT INFORMATION (CHANNEL 18)
    ...
    % RE-REFERENCE TO AVERAGE MASTOIDS AND ADD Cz BACK. NEED TO DOUBLE
    % CHECK THIS ONE (COORDINATES OF Cz MAY BE INCORRECT)
    EEG = pop_reref( EEG, [5 6], 'refloc', struct('labels', ...
        {'Cz'}, 'theta', {90}, 'radius', {0}, ...
        'X', {3.7494e-33}, 'Y', {-6.1232e-17}, 'Z', ...
        {1}, 'sph_theta', {-90}, 'sph_phi', {90}, 'sph_radius', ...
        {1}, 'type', {''}, 'ref', {'Cz'}, ...
        'urchan', {33}, ...
        'datachan', {0}, ...
        'sph_theta_besa', {0}, ...
    ...
    % EXTRACT 1 MINUTE SEGMENTS OF EC 
    ...
    % EXTRACT 1 MINUTE SEGMENTS OF EO
    ...
    %% ARTIFACT REMOVAL WITH ASR ON EC DATA
    
    % LOAD EC DATA
    EEG_EC = pop_loadset('filename',[subject '_EC.set'],'filepath', ecdir);
    ...
    EEG_EC.setname = [subject '_EC_ASR']; % NAME FOR DATASET MENU
    
    % STORE REMOVED CHANNELS FOR REVIEW
    newchans_EC = {EEG_EC.chanlocs.labels}; % SAVE NEW EO CHANS AFTER CLEAN
    chandiff_EC = setdiff(oldchans, newchans_EC); % DIFFERENCE OLD AND NEW CHANNELS
    interchans_EC(s) = {chandiff_EC}; % STORE LIST OF INTERPOLATED CHANNELS FOR EACH SUBJECT

    % SAVE CLEANED RS DATA FOR VISUAL EXAMINATION
    if (save_everything)
        EEG_EC = pop_saveset(EEG_EC, 'filename',[subject '_EC_ASR.set'], ...
            'filepath', ecdir);
    end
    ...
    % LOAD EO DATA
    EEG_EO = pop_loadset('filename',[subject '_EO.set'],'filepath', eodir);
   ...
   % SAVE ORIGINAL DATA BEFORE REMOVING BAD CHANNELS
    ...
    EEG_EO = pop_clean_rawdata(EEG_EO, 'FlatlineCriterion', 'off', ...
        ...
        'Distance', 'Euclidian', ...
        'WindowCriterionTolerances', [-Inf 7] );
    EEG_EO.setname = [subject '_EO_ASR']; % NAME FOR DATASET MENU
    
    % STORE REMOVED CHANNELS FOR REVIEW
    newchans_EO = {EEG_EO.chanlocs.labels}; % SAVE NEW EO CHANS AFTER CLEAN
    chandiff_EO = setdiff(oldchans, newchans_EO); % DIFFERENCE OLD AND NEW CHANNELS
    interchans_EO(s) = {chandiff_EO}; % STORE LIST OF INTERPOLATED CHANNELS FOR EACH SUBJECT

    % SAVE CLEANED EO DATA FOR VISUAL EXAMINATION
    if (save_everything)
        EEG_EO = pop_saveset(EEG_EO, 'filename',[subject '_EO_ASR.set'], ...
            'filepath', eodir);
    end
    ...
    EEG_EC.setname = [subject '_EC_ICA_Weights']; % NAME FOR DATASET MENU
    EEG_EO.setname = [subject '_EO_ICA_Weights']; % NAME FOR DATASET MENU
    EEG_EC = eeg_checkset(EEG_EC, 'ica');
    EEG_EO = eeg_checkset(EEG_EO, 'ica');
      
    % SAVE DATA WITH ICA WEIGHTS
    if (save_everything)
        EEG_EC = pop_saveset(EEG_EC, 'filename',[subject '_EC_ICA_Weights.set'], ...
            'filepath', ecdir);
        EEG_EO = pop_saveset(EEG_EO, 'filename',[subject '_EO_ICA_Weights.set'], ...
            'filepath', eodir);
    end
    
    % RUN ICLABEL(Pion-Tonachini et al., 2019) TO LABEL COMPONENTS
    EEG_EC = pop_iclabel(EEG_EC, 'default');
    EEG_EC = pop_iclabel(EEG_EO, 'default');
    ...
    % CREATE EO AND EC EPOCHS OF 2 SEC, WITH 75% OVERLAP (0.5 s RECURRENCE) 
