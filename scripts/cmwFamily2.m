function [wavefam,wavefam2] = cmwFamily2(varargin)
...
for fi=1:length(frex)
    ...
    h              = (nCycles(fi)*sqrt(2*log(2))) / (pi*frex(fi)); % fwhm
    wavefam(fi,:)  = exp(2*1i*pi*frex(fi).*wavtime) .* exp( (-wavtime.^2) ./ (2*s^2) );
    wavefam2(fi,:) = exp(2*1i*pi*frex(fi).*wavtime) .* exp( (-4*log(2)*wavtime.^2) / (h^2) );
end
