...
pairs(dataF2F1[6:10], panel = panel.lm,
...
# BOXPLOTS FOR FAA OUTLIERS - https://towardsdatascience.com/how-to-detect-unusual-observations-on-your-regression-model-with-r-de0eaa38bc5b

p <- ggplot(dataF2F1, aes(factor(sub.type), val)) 
p + geom_boxplot() + facet_grid(. ~ dataF2F1.type)

lmts <- range(Data$FAA.F2F1, Data$FAA.F4F3, Data$FAA.F6F5, Data$FAA.F8F7)
boxplot(Data$FAA.F2F1, ylim=lmts)
boxplot(Data$FAA.F4F3, ylim=lmts)
boxplot(Data$FAA.F6F5, ylim=lmts)
boxplot(Data$FAA.F8F7, ylim=lmts)
